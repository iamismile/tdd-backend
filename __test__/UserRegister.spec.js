...
describe('Internationalization', () => {
  it.each`
    ...
    ${'password'} | ${'lowerUPPER'}    | ${tr.password_pattern}
    ${'password'} | ${'lower12345'}    | ${tr.password_pattern}
    ${'password'} | ${'UPPER12345'}    | ${tr.password_pattern}
  `(
  ...
  it(`returns ${tr.email_failure} when sending email fails and language is set as turkish`, async () => {
    simulateSmtpFailure = true;
    const response = await postUser({ ...validUser }, { language: 'tr' });
    expect(response.body.message).toBe(tr.email_failure);
  });

  it(`returns ${tr.validation_failure} message in error response body when validation fails and langiage is set as turkish`, async () => {
